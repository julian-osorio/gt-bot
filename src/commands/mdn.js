...
async function mdnLookup(query) {
  ...
  let queryFormatted = queryArray.join('+');
  ...
  try {
    ...
    let sliceResArr = resArray.slice(0, 3);

    let topResults = sliceResArr.map(result => { 
      return {"title": result.title, "url": makeUrl(result.mdn_url)}
    });
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      let searchResults = await mdnLookup(query);
